...
class CamVidDataset(Dataset):
    def __init__(self, args, val=False, query=False):
        ...
        self.crop_size, self.pad_size = (384, 480), (0, 0)
        ...
        if n_pixels_per_img != 0 and not val:
            ...
            label = Image.open(self.list_labels[0]).resize((self.crop_size[1], self.crop_size[0]), Image.NEAREST)
            w, h = label.size

            # iterate over labels to get masks
            for i in tqdm(range(len(self.list_labels))):
                label = np.array(Image.open(self.list_labels[i]).resize((self.crop_size[1], self.crop_size[0]), Image.NEAREST))  # H x W
    ...
    def __getitem__(self, ind):
        ...
        x = Image.open(self.list_inputs[ind]).convert("RGB").resize((self.crop_size[1], self.crop_size[0]), Image.BILINEAR)
        y = Image.open(self.list_labels[ind]).resize((self.crop_size[1], self.crop_size[0]), Image.NEAREST)
