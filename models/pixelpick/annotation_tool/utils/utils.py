...
alphabet_metalDAM = ['A', 'M', 'R', 'P', 'D', ]
alphabet_voc = ['Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P', 'A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'Z', 'X']

alphabet_l = [a.lower() for a in alphabet]
alphabet_l_cv, alphabet_l_voc, alphabet_l_metalDAM = \
    [a.lower() for a in alphabet_cv], [a.lower() for a in alphabet_voc],  [a.lower() for a in alphabet_metalDAM],
...
metalDAM_label_category = {
    0: "austenite",
    1: "matrix",
    2: "maRtensite/austenite(MA)",
    3: "precipitates",
    4: "defect",
}
...
def annotate(arr, dataset_name):
    ...
    alphabet_l = alphabet_l_cv if dataset_name == "camvid" \
        else alphabet_l_metalDAM if dataset_name == "metalDAM" else alphabet_l_voc
    while True:
        k = cv2.waitKey()
...
def make_gui(img, dataset_name, **kwargs):
    ...
    label_category = cv_label_category if dataset_name == "camvid" \
        else metalDAM_label_category if dataset_name == "metalDAM"\
        else voc_label_category
    alphabet = alphabet_cv if dataset_name == "camvid" else alphabet_metalDAM if dataset_name == "metalDAM"  else alphabet_voc
...
class Logger:
    ...
    def __call__(self, fname, line, mode):
        ...
        with open(f"{os.path.join(self.dir_log, fname)}.txt", mode) as f:
            f.write(line)
            f.close()
